...
class GameWindow():
    def __init__(self, agent_white='human', agent_black='human'):
        ...
        self.canvas = tk.Canvas(self.window, bg="green", width=self.cell_size*8,
        ...
        for i in range(8):
            for j in range(8):
                ...
                self.canvas.create_line(x_min, y_min,
                                        x_max, y_min,
                                        x_max, y_max,
                                        x_min, y_max)
    ...
    def update_game_state(self, state):
        for i in range(8):
            for j in range(8):
                if state[i][j] == WHITE:
                    cell_box = np.array([i, j, (i + 1), (j + 1)]) * self.cell_size + 2
                    self.canvas.create_oval(cell_box[0], cell_box[1],
                                            cell_box[2], cell_box[3], fill="white")
                elif state[i][j] == BLACK:
                    cell_box = np.array([i, j, (i + 1), (j + 1)]) * self.cell_size + 2
                    self.canvas.create_oval(cell_box[0], cell_box[1],
                                            cell_box[2], cell_box[3], fill="black")
    ...
    def callback(self, event):
        ...
        if ((coord[0] > 8*self.cell_size-1 or coord[0] < 0) or
           (coord[1] > 8*self.cell_size-1 or coord[1] < 0)):
            ...
            state, player_turn, game_result = self.reversi.step(board_coord, matrix_coord=True)
            self.update_game_state(state)
            if game_result == 0:
                print("It's a draw!")
            elif game_result == 1:
