...
class ReversiEnvironment:
    ...
    def step(self, location):
        """
        A player makes a move, updating the game state. Returns states.
        :param location:
        :return:
        """
...
class GameBoard:
    ...
    # TODO: Winning condition, no valid moves left, keeping track of player turn
    ...
    def place_piece(self, coord, player):
        ...
        :return:
        """
        if self.is_valid_placement(coord, player):
    ...
    def is_valid_placement(self, coord, player):
        """
        ...
        if n > 1:
            for i in range(n-1):
                if self.board[self.coordinates_to_matrix((x+i+1, y+i+1))] == EMPTY:
                    ...
                    northeast_list = northeast_list + [list(self.coordinates_to_matrix((x+i+1, y+i+1)))]
        if is_valid:
        ...
        if n > 1:
            for i in range(n-1):
                if self.board[self.coordinates_to_matrix((x+i+1, y-i-1))] == EMPTY:
                    ...
                    southeast_list = southeast_list + [list(self.coordinates_to_matrix((x+i+1, y-i-1)))]
        if is_valid:
        ...
        if n > 1:
            for i in range(n-1):
                if self.board[self.coordinates_to_matrix((x-i-1, y-i-1))] == EMPTY:
                    ...
                    southwest_list = southwest_list + [list(self.coordinates_to_matrix((x-i-1, y-i-1)))]
        if is_valid:
        ...
        if n > 1:
            for i in range(n-1):
                if self.board[self.coordinates_to_matrix((x-i-1, y+i+1))] == EMPTY:
                    ...
                    northwest_list = northwest_list + [list(self.coordinates_to_matrix((x-i-1, y+i+1)))]
        if is_valid:
